...
export class JwtService {
  ...
  async parseToken(token: string, sessionId: string) {
    try {
      const jwtToken: Member | Jwt | JwtPayload | string | number = await this.verifyToken(String(Decrypt(token)));
      const member = jwtToken as TokenMember;
      const sessionInfo = await this.sessionService.checkSession(member.idx, sessionId);
